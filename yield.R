...
detrend <- lm(corn ~ t + t2, data = yield1)
yield1$dtrnd_c <- p_c_2017*(1 + detrend$resid/detrend$fitted.values)

detrend <- lm(soybeans ~ t + t2, data = yield1)
yield1$dtrnd_s <- p_s_2017*(1 + detrend$resid/detrend$fitted.values)

yield1 <- filter(yield1, year > 1950)
ggplot() +
  geom_line(data = yield1, aes(x=year, y=dtrnd_c, group = 1), color = "black")

ggplot() +
  geom_line(data = yield1, aes(x=year, y=dtrnd_s, group = 1), color = "black")
